...
this_project_root = Path(__file__).parent.parent

datasets_root = this_project_root.parent / 'datasets'
...
class MIMIC_E_T_Dataset(Dataset):
    def __init__(self, ecg_meta_path, transform=None, **args):
        ...
        self.text_csv = args['text_csv']
        self.label_cols = list(self.text_csv.columns[6:]) if isinstance(self.text_csv, pd.DataFrame) else []
    ...
    def __getitem__(self, idx):
        ...
        if len(self.label_cols) > 0:
            labels = self.text_csv.iloc[idx][self.label_cols].values.astype(np.float32)
        else:
            labels = np.array([], dtype=np.float32)

        sample = {'ecg': ecg, 'report': report, 'labels': labels}
