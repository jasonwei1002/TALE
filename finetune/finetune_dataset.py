...
class ECGDataset(Dataset):
    def __init__(self, data_path, csv_file=None, mode='train', dataset_name='ptbxl', ratio=100, backbone='resnet18'):
        """
        Args:
            data_path (string | Path | DataFrame): Path to store raw data or csv path.
            csv_file (string | Path | DataFrame | None): .csv file with labels and data path.
        ...
        if mode == 'train' and ratio != 100:
            if ratio <= 0 or ratio > 100:
                raise ValueError("ratio should be in (0, 100]")
            csv_file, _ = train_test_split(csv_file, train_size=(ratio / 100), random_state=42)
        ...
        if self.dataset_name.startswith('ptbxl'):
            ...
            raise ValueError("dataset_name should be either 'ptbxl', 'icbeb', or 'chapman'")
    ...
    def __getitem__(self, idx):
        if self.dataset_name.startswith('ptbxl'):
            ...
            raw_path = self.ecg_path[idx]
            if self.data_path:
                if os.path.isabs(raw_path) and os.path.exists(raw_path):
                    ecg_path = raw_path
                else:
                    ecg_path = os.path.join(self.data_path, str(raw_path).lstrip(os.sep))
            else:
                ecg_path = raw_path
