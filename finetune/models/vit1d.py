...
class VisionTransformer(nn.Module):
    def __init__(self, config: Config | None = None,
                 keep_registers: bool = False, use_sdp_kernel: bool = True, num_classes: int = 0):
        super().__init__()
        if config is None:
            config = Config()
        self.config = config
        ...
        self.attentive_pooler = AttentivePooler(
            ...
            num_queries=config.num_registers,
            depth=2,
        ...
        if config.num_registers > 0:
            ...
            nn.init.trunc_normal_(self.registers, mean=0.0, std=0.02)
        self.blocks = nn.ModuleList([
            Block(
                dim=config.dim,
                num_heads=config.num_heads,
        ...
        if self.num_classes > 0:
            self.head = nn.Linear(config.dim, self.num_classes)
            self.linear = self.head
            ...
            if isinstance(module, (nn.Linear, nn.Conv1d)):
                if name.endswith('mlp.fc2') or name.endswith('attn.proj'):
                    ...
                    nn.init.trunc_normal_(
                        module.weight,
                        mean=0.0,
                        std=0.02 / math.sqrt(2 * config.depth),
                    )
                else:
    ...
    def reset_head(self, num_classes: int):
        self.num_classes = int(num_classes)
        self.head = nn.Linear(self.config.dim, self.num_classes)
        self.linear = self.head


def vit_small(config: Config | None = None,
              keep_registers: bool = False, use_sdp_kernel: bool = True,
              num_classes: int = 0):
